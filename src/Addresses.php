...
class Addresses_Customer extends Extension
{
    ...
    public function createAddresses($order)
    {
        ...
        if (!$this->owner->FirstName) {
            if (!empty($data['ShippingFirstName'])) {
                $this->owner->FirstName = $data['ShippingFirstName'];
            } elseif (!empty($data['BillingFirstName'])) {
                $this->owner->FirstName = $data['BillingFirstName'];
            } else {
                $this->owner->FirstName = 'Unnamed';
            }
            $this->owner->write();
        }
        if (!$this->owner->Surname) {
            if (!empty($data['ShippingSurname'])) {
                $this->owner->Surname = $data['ShippingSurname'];
            } elseif (!empty($data['BillingSurname'])) {
                $this->owner->Surname = $data['BillingSurname'];
            } else {
                $this->owner->Surname = 'Account';
            }
            $this->owner->write();
        }
